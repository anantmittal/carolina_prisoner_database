...
varnames_first="LASTNAME\t"
varnames_second="LASTNAME\t"
...
# Get last names out of file
lines = [line.rstrip('\r\n').split('\t') for line in open('testname.csv', 'r')]
last_names = [lines[j][0] for j in xrange(1, len(lines))]

# Get data from each link
data_text = ""

# Writing the data to file
fw_first = open("FirstPrisonDatabogbe6.csv", 'w')
fw_first_error = open("FirstPrisonData_ERRORbogbe6.csv", 'w')
fw_first.write(varnames_first) # Write variable names

fw_second = open("SecondPrisonDatabogbe6.csv", 'w')
fw_second_error = open("SecondPrisonData_ERRORbogbe6.csv", 'w')
...
for link_j in links_j:
    for last_name in last_names:
        print str(counter) + "\t" + link_j + "\t" + last_name
        counter += 1
        br = Browser()
        try:
            br.open(link_j)
        ...
        for l in br.links():
            ...
            if l1 != "http://www.ramajudicial.gov.co":  # Not useful
                ...
                try:
                    data_text = str(last_name) + "\t" + get_data_first(l1)
        ...
        for link in links_first:
            ...
            for l in br.links():
                l2 = l.absolute_url
                try:
                    data_text = str(last_name) + "\t" + get_data_second(l2)
